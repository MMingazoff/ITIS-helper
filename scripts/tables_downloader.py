...
sheet = None
path = os.path.abspath(__file__)[:-28]
...
def start_up():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(path + 'data/credentials.json', SCOPES)
...
def download_raskraska():
    ...
    with pd.ExcelWriter(path + "data/timetable/timetable.xlsx") as writer:
    ...
    for dataframe, SAMPLE in zip(dataFrame_of_raskraska[1:], SAMPLE_RANGE_NAME_raskraska[1:]):
        with pd.ExcelWriter(path + "data/timetable/timetable.xlsx", engine='openpyxl', mode='a') as writer:
